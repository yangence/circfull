...
def filterBS_fusion2(genomeFile,gtfFile,outPrefix):
    ...
    BS_Fusion2=pd.DataFrame([mapExon(BS_Fusion2.iloc[i].copy())[0] for i in range(BS_Fusion2.shape[0])])
    BS_Fusion2_left=BS_Fusion2[['ID','leftSchr','leftEchr','leftSpos','leftEpos','leftSseq','leftEseq','strand']].copy()
    BS_Fusion2_right=BS_Fusion2[['ID','rightSchr','rightEchr','rightSpos','rightEpos','rightSseq','rightEseq','strand']].copy()
    BS_Fusion2_left.columns=['ID','chr1','chr2','start','end','leftSeq','rightSeq','strand']
    BS_Fusion2_right.columns=['ID','chr1','chr2','start','end','leftSeq','rightSeq','strand']
    BS_Fusion2_adj=pd.DataFrame([(adjustBS(BS_Fusion2_left.iloc[i])+'\t'+adjustBS(BS_Fusion2_right.iloc[i],type='type2')).split('\t') for i in range(BS_Fusion2_left.shape[0])],columns=['ID','leftChr','rightChr','leftSpos','leftEpos','leftSseq','leftEseq','rightSpos','rightEpos','rightSseq','rightEseq','strand'])
    BS_Fusion2_adj=BS_Fusion2_adj.sort_values(['leftChr','rightChr','leftSpos','leftEpos'])
    BS_Fusion2_adj['circID']=BS_Fusion2_adj['leftChr']+'|'+BS_Fusion2_adj['rightChr']+'|'+BS_Fusion2_adj['leftSpos']+'|'+BS_Fusion2_adj['leftEpos']+'|'+BS_Fusion2_adj['rightSpos']+'|'+BS_Fusion2_adj['rightEpos']
    BS_Fusion2_adj.to_csv(outPrefix+'BS_Fusion2_adj.txt',sep='\t',index=None)
    BS_Fusion2_adj_1=BS_Fusion2_adj.loc[:,['ID','leftChr','rightEpos','leftSpos','rightEseq','leftSseq']].copy()
    BS_Fusion2_adj_1.to_csv(outPrefix+'BS_Fusion2_adj_1.txt',sep='\t',index=None,header=None)
    leftList=[]
    rightList=[]
    leftListS=[]
    rightListS=[]
    for i in range(BS_Fusion2_adj.shape[0]):
        tmp=BS_Fusion2_adj.iloc[i]
        if tmp['strand'] == 'True':
