...
def fusion2AdjFL(options):
    ...
    consFLraw=pd.read_csv(outPrefix+'constructFL_Fusion2_'+adjFusionNum+'.txt',sep='\t')
    isSecond=False
    if len(options)>5:
        isSecond=True
    ...
    newDf.to_csv(outPrefix+'constructFL_Fusion2_adj_'+adjFusionNum+'.txt',sep='\t',index=None)